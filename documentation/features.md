...
* **定期檢驗 / 排氣定檢 (`inspection`)**：
  * 依台灣監理規範，法定應檢期間以行照**原發照日期 (`registration_date`)** 的月日為基準，前後各 1 個月（共 2 個月檢驗窗口）；
  * 汽車滿 5 年未滿 10 年每年 1 次，10 年以上每年 2 次；未滿 5 年新車免定檢（系統顯示預估窗口提示）；
  * 機車滿 5 年每年排氣定檢 1 次（規費預設為 0 元）；
* **強制汽車責任險 (`compulsory_insurance`) & 任意險 (`liability_insurance`)**：支援自訂覆蓋起訖日，預設 1 年保單週期。

### 智慧預填與連動安全性 (Smart Pre-fill & Overwrite Guard)
* **法規優先推算**：優先依 `registration_date` 之月日推算檢驗視窗，車齡依 `manufacture_date`（或 `year`）計算。
* **Fallback 降級機制**：若車輛未填 `registration_date`，自動退回以 `manufacture_date` 之 1 號為預估基準，並提示車主儘速補填以利精確推算。
* **覆寫警示與原子更新**：登記檢驗時勾選「同步設為行照原發照日期」：
  * 若車輛已設有不同之原發照日，彈出二次確認視窗 (`Alert.alert`) 防止誤觸；
  * 後端在單一原子交易 (`pgx.Tx`) 內完成規費表記錄寫入與車輛表 `registration_date` 更新。
