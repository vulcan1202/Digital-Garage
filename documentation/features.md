...
* **核心規格屬性**：記錄品牌、型號、出廠日期（`manufacture_date`）、年份（`year`）、車牌號碼、排氣量（cc）、動力燃料種類、購入日期（`purchase_date`）與購入金額（`purchase_price`）。
* **出廠日期與年份自動解析 (P2-2.5)**：全面採用 Dark-Metal 日期選擇器選定出廠日，自動推算西元年份並回傳後端，兼具向後相容性與資料庫完整性約束。
...
* **定期檢驗 / 排氣定檢 (`inspection`)**：
  * 汽車滿 5 年未滿 10 年每年 1 次，10 年以上每年 2 次；
  * 機車滿 5 年每年排氣定檢 1 次；
  * 法定應檢期間為行照出廠日（或原發照日）**前後各 1 個月**（共 2 個月檢驗窗口）。
